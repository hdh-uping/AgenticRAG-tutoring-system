...
图中显示的是教材中的小节编号“2.1”，通常表示第二章第一节的内容，用于引导读者进入该章节的学习。
...
图中显示的是数字“2.2”，通常表示教材或文档中的章节或小节编号，用于标识相关内容的位置。
...
该图展示了数组在插入操作前的状态，数据元素从序号0到listsize-1依次排列，图中标注了从后往前移动的箭头，暗示插入操作将从末尾开始向前调整元素位置。

该图展示了在数组中插入元素 e 后的数据结构变化，原位置 i 处的元素被向后移动，新元素 e 插入在位置 i，序号从 0 到 listsze-1，图中标注了插入前后各元素的序号和位置。  
图 2.2 在顺序表中插入一个元素
...
该图展示了线性表在删除操作前的状态，序号从0到MaxSize-1，每个序号对应一个数据元素，箭头指示从前往后移动的方向。  
(a) 删除前

该图展示了一个线性表的数据结构示意图，其中数据元素按序号从0到MaxSize-1排列，当前有效数据元素为a₀至a_{n-1}，序号i对应元素a_{i+1}。  
(b) 删除后  
图 2.3 从顺序表中删除一个元素
...
图中展示了一个节点结构，包含“data”和“next”两个字段，常用于链表数据结构的示意图。  
图 2.4 单链表节点结构
...
该图展示了一个线性结构，从左到右依次为初始符号L，接着是包含元素a₀和一个黑点的节点，然后是a₁和一个黑点的节点，以此类推，最后是a_{n-1}和一个黑点，整体表示一个序列或链表结构。  
图 2.5 单链表示意图
...
图中展示了一个链表节点q，其数据域q->data和指针域q->next指向下一个节点，形象地表示了链式存储结构的基本组成。  
图 2.6 申请一个节点
...
该图展示了多条链表结构，每条链表从左至右依次经过节点变换，最终指向值为35的节点，其中节点值在变换过程中呈现特定规律，如67→28→48→35等，体现了数据结构中链表的动态更新或遍历过程。  
图 2.7 头插入法建立单链表示意图
...
该图展示了链表在插入或遍历过程中节点的动态变化过程，每个步骤中节点通过箭头连接形成链式结构，同时标有指针p指向当前操作节点。  
图 2.8 尾插入法建立单链表示意图
...
该图展示了单链表的两种状态：(a) 空表，仅有一个头节点（H）指向空；(b) 非空表，头节点指向第一个节点a₀，依次链接至a₁、...、aₙ₋₁，最后节点指向空。  
图 2.9 带头节点的单链表
...
该图展示了两个链表操作场景：左图中指针p指向链表节点，s指向一个独立节点e，p指向的节点被删除后，s指向的节点e被插入到链表中；右图中指针p和q分别指向链表中不同位置，s指向节点e，q指向的节点被删除后，s指向的节点e被插入到链表中。  
图 2.10 在节点之后插入  
图 2.11 在节点之前插入
...
该图展示了链表中节点的结构与指针关系，其中p、q、s分别指向不同节点，箭头表示指针指向下一个节点，同时显示了节点间的连接方式和数据存储结构。  
图 2.12 删除链表中的节点
...
该图展示了一个循环结构，输入H经过处理后生成一系列状态a₀、a₁、...、a_{n-1}，每个状态后又反馈回前一处理单元，形成闭环处理流程。  
图 2.13 带头节点的单向循环链表
...
该图展示了两个链表结构，每个链表由带斜线的头节点和若干数据节点组成，节点间通过箭头连接，同时存在从尾部节点指向头节点的循环指针，用于实现循环链表结构。  
...
该图展示了一个循环链表的数据结构示意图，其中包含一个头节点（带斜线填充）、多个数据节点（标记为a₀、a₁、...、aₙ₋₁）以及指向各节点的指针箭头，形成闭环结构。  
图 2.16 带头节点的双向循环链表
...
该图展示了一个双向链表结构，其中包含多个节点，节点间通过双向箭头连接，图中标注了p、r、s等指针或位置，用于指示链表中特定节点或操作点，体现了链表的遍历或修改机制。  
...
该图展示了一个双向链表结构，其中包含多个节点，节点间通过双向箭头连接，图中还标示了指针r、p和s，以及一个中间节点的特殊处理，可能用于演示链表的插入、删除或遍历操作。  
...
该图展示了一个链表结构，其中包含多个节点，节点间通过箭头连接形成链式结构，同时有三个标记为p、q、r的指针指向特定节点，用于演示链表中节点的插入或删除操作。  
...
该图展示了一个顺序表（线性表）在插入和删除元素后的操作过程，包括元素的初始状态、插入位置0后变为30、1、2、3、4、15，删除位置4后变为30、1、2、3、15，以及查找36失败和读取位置3元素值为3的最终结果。  
...
该图展示了一个单链表在插入和删除操作后的元素变化过程，初始元素为1至6，插入5后变为1至6，删除5后变为1至6，同时显示了线性表中查找元素1的位置为0。  
(a) 查找成功

该图展示了一个单链表的输入、插入和删除操作过程，输入长度为6的链表后，插入值7并删除值5，最终链表元素变为1、2、3、4、6，且提示查找值7不存在。  
(b) 查找失败  
...
这张图片展示了一个通讯录管理程序的控制台界面，用户通过菜单选项进行联系人信息的建立、插入、查询、删除和输出操作，界面以文本形式呈现数据结构和交互过程。  
图 2.22 通讯录程序的运行结果